...
def processhyperkinetic_dysphonia(path):
	mfcc=[]
	name=[]
	clas=[]
	sample=[]
	...
	for i in range(0,len(mfcc)):
		...
		full.append(d)
	print(full)
	with open('covid.csv', 'w',newline='') as myfile:
	    wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
	    for x in full:
	        wr.writerow(x)

def processHealthy(path):
	mfcc=[]
	name=[]
	clas=[]
	sample=[]
	...
	for i in range(0,len(mfcc)):
		...
		full.append(d)
	print(full)
	with open('noncovid.csv', 'w',newline='') as myfile:
	    wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
	    for x in full:
	        wr.writerow(x)

def process(path):	        
	processHealthy(path+"/Noncovid")
	processhyperkinetic_dysphonia(path+"/Covid")
	
	reader = csv.reader(open("noncovid.csv"))
	reader1 = csv.reader(open("covid.csv"))
